...
def update_user(user_id: int, login: str, password: str, notification_subscribe: bool = True, term: int = 1) \
        -> None:
    r"""Adds user into database_; if 'user_id' is already in database_, only updates login and password,
    ...
    make_sql_query('INSERT INTO user_info(tg_id, login, password, notifications, term) VALUES (%s, %s, %s, %s, %s)'
                   'ON DUPLICATE KEY UPDATE login = %s, password = %s',
                   (user_id, login, password, notification_subscribe, term,
                    login, password))


def update_student_works(works: list[WorkInfo], login: str) -> None:
    r"""Updates works info in database_. Creates new info, if work doesn't yet exist in database_

    :param works: list of :class:`WorkInfo` objects with paramaters to be updated in database_
...
def update_user_notification_subscribe(user_id: int, notification_subscribe: bool) -> None:
    r"""Changes database_ info about notification subsсription for user"""
...
def update_student_grades(grades_info: list[GradeInfo], login: str = None, user_id: int = None) -> None:
    ...
    If both are given, only 'login' is used. Updates already existing works in database_. If work doesn't exist,
    adds new work"""
