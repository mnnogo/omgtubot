...
def is_user_authorized(user_id: int) -> bool:
    ...
    user_encounters = make_sql_query('SELECT COUNT(*) FROM user_info WHERE user_id = ? AND login IS NOT NULL',
                                     (user_id,))[0][0]
...
def is_user_subscribed_notifications(user_id: int) -> bool:
    ...
    result = make_sql_query('SELECT notifications FROM user_info WHERE user_id = ?', (user_id,))
...
def is_user_subscribed_mailing(user_id: int) -> bool:
    ...
    result = make_sql_query('SELECT mailing FROM user_info WHERE user_id = ?', (user_id,))
